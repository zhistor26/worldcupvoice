...
/** True when the private-preview password gate should be enforced. */
export function isAccessGateEnabled(): boolean {
  if (process.env.LAZYCAT_DEPLOYED === 'true') return false;
  if (process.env.NEXT_PUBLIC_LAZYCAT_DEPLOYED === 'true') return false;
  if (process.env.NEXT_PUBLIC_REQUIRE_ACCESS_PASSWORD === 'false') return false;
  return isAccessPasswordConfigured();
}
