...
def _settings_allow_profile_tts() -> Settings:
    return Settings(
        agora_app_id="app-id",
        agora_app_certificate="app-cert",
        vision_provider="openai",
...
async def test_start_uses_selected_commentator_profile(monkeypatch):
    ...
    monkeypatch.setattr("app.session_manager.BackendVisionCommentator", FakeCommentator)
    manager = SessionManager(_settings_allow_profile_tts())
...
@pytest.mark.asyncio
async def test_default_profile_uses_global_mimo_tts(monkeypatch):
    monkeypatch.setattr("app.session_manager.BackendVisionCommentator", FakeCommentator)
    manager = SessionManager(_settings())
    ...
    assert FakeCommentator.instances[0].kwargs["settings"].tts_provider == "mimo"
    await manager.close()
