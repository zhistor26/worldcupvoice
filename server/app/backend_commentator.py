...
_DEV_TEST_FEED_LINES = (
    "演播室信号正常，彩条就位，导播准备切入正式比赛画面！",
...
def _mimo_auth_headers(api_key: str) -> dict[str, str]:
    return {
        "api-key": api_key,
...
def _extract_mimo_tts_audio(payload: dict) -> bytes:
    choices = payload.get("choices")
    if not isinstance(choices, list) or not choices:
        return b""
    message = choices[0].get("message", {})
...
def _transcript_payload(
    *,
    ...
    turn_id: int,
    language: str = "zh-CN",
    turn_status: int = TRANSCRIPT_TURN_END,
) -> bytes:
    payload = {
...
def _build_visual_prompt(
    ...
    *,
    ...
    if profile and profile.language.startswith("zh"):
        dev_test_feed_note = ""
        if settings and settings.commentary_dev_test_feed:
            dev_test_feed_note = (
                "\n若画面是测试彩条、导播测试图或技术校色信号，不要返回 NO_CALL；"
                "用一句有梗的中文直播口吻说明信号正常、等待正式比赛画面即可。\n"
            )
        return (
            "你是实时足球解说员，不是图片说明员。\n"
...
def _normalize_commentary(text: str) -> str:
    # Keep CJK so Chinese commentary is not stripped to empty (which broke dedupe).
...
def _is_repetitive_commentary(
    text: str,
    previous_calls: list[str],
    *,
    last_spoken_text: str | None = None,
    last_spoken_monotonic: float = 0.0,
    min_repeat_interval_s: float = 3.0,
) -> bool:
    """Only debounce duplicate back-to-back commentary, not looped clips."""
    del previous_calls
    ...
    if normalized != _normalize_commentary(last_spoken_text):
        return False

    return time.monotonic() - last_spoken_monotonic < min_repeat_interval_s
...
class BackendVisionCommentator:
    ...
    def _next_dev_test_feed_line(self) -> str:
        line = _DEV_TEST_FEED_LINES[self._dev_fallback_index % len(_DEV_TEST_FEED_LINES)]
    ...
    async def _commentary_from_latest_frames(self, connection: object) -> None:
        ...
        try:
            ...
            if _is_repetitive_commentary(
                ...
                last_spoken_monotonic=self._last_spoken_monotonic,
            ):
            ...
            await self._stream_transcript_cjk(connection, text, turn_id=turn_id)
    ...
    def _pause_after_openai_rate_limit(self, retry_after: str | None) -> None:
        ...
        logger.warning("Vision provider rate limited; pausing commentary for %.1fs", delay)
    ...
    async def _describe_frames(self, samples: list[FrameSnapshot]) -> str:
        if self._settings.vision_provider == "mimo":
            return await self._describe_frames_mimo(samples)
        return await self._describe_frames_openai(samples)

    async def _describe_frames_mimo(self, samples: list[FrameSnapshot]) -> str:
        ...
        request_body = {
            "model": self._settings.mimo_vision_model,
            "messages": [{"role": "user", "content": content}],
            # mimo-v2.5 spends many tokens on reasoning_content; 40 leaves content empty.
            "max_tokens": 512,
            "temperature": 0.55,
        }
        url = f"{self._settings.mimo_base_url}{MIMO_CHAT_COMPLETIONS_PATH}"

        async with httpx.AsyncClient(timeout=35) as client:
            response = await client.post(
                url,
                headers=_mimo_auth_headers(self._settings.mimo_api_key),
                json=request_body,
            )
            ...
            logger.info(
                "Generated MiMo visual commentary model=%s: %s",
                self._settings.mimo_vision_model,
                text,
            )
            return text

    async def _describe_frames_openai(self, samples: list[FrameSnapshot]) -> str:
    ...
    async def _synthesize_speech(self, text: str) -> bytes:
        if self._settings.tts_provider == "mimo":
            if self._settings.mimo_api_key:
                try:
                    return await self._synthesize_speech_mimo(text)
                except Exception:
                    logger.warning(
                        "MiMo TTS failed; falling back to other TTS providers",
                        exc_info=True,
                    )
            else:
                logger.warning(
                    "TTS_PROVIDER=mimo requires MIMO_API_KEY; falling back to other TTS providers"
                )
        ...
        raise RuntimeError("No TTS provider is configured for commentary audio.")
    ...
    async def _synthesize_speech_mimo(self, text: str) -> bytes:
        if not self._settings.mimo_api_key:
            raise RuntimeError("MIMO_API_KEY is required for MiMo TTS.")

        style_hint = (
            f"用{self._profile.language}足球直播解说语气朗读，节奏快、有现场感。"
            if self._profile
            else "中文足球直播解说语气，节奏快、有现场感。"
        )
        request_body = {
            ...
            "audio": {
                "format": "pcm16",
                "voice": self._settings.mimo_tts_voice,
            },
        }
        url = f"{self._settings.mimo_base_url}{MIMO_CHAT_COMPLETIONS_PATH}"
        ...
        async with httpx.AsyncClient(timeout=45) as client:
            ...
            response.raise_for_status()
            audio_pcm = _extract_mimo_tts_audio(response.json())
            if not audio_pcm:
    ...
    def _tts_description(self) -> str:
        if self._settings.tts_provider == "mimo":
            return f"mimo:{self._settings.mimo_tts_model}:{self._settings.mimo_tts_voice}"
        if self._settings.tts_provider == "elevenlabs":
    ...
    async def _publish_transcript(
        ...
        *,
        ...
        turn_status: int = TRANSCRIPT_TURN_END,
        advance_turn_id: bool = True,
    ) -> None:
        active_turn_id = self._turn_id if turn_id is None else turn_id
        language = "zh-CN"
        if self._profile and self._profile.transcript_language:
            language = (
                "zh-CN"
                if self._profile.transcript_language.startswith("zh")
                else self._profile.transcript_language
            )
        ...
        if ret != 0:
            logger.warning(
                "send_stream_message returned %s for turn_id=%s",
                ret,
                active_turn_id,
            )
        if advance_turn_id:
    ...
    async def _stream_transcript_cjk(
        self,
        connection: object,
        text: str,
        *,
        ...
        """Stream Simplified Chinese subtitles token-by-token via RTC data channel."""
        cleaned = text.strip()
        if not cleaned:
            return
        ...
        await self._publish_transcript(
            ...
            cleaned,
            turn_id=turn_id,
            turn_status=TRANSCRIPT_TURN_IN_PROGRESS,
            advance_turn_id=False,
        )

        # CHUNK render mode types out the line character-by-character.
        await asyncio.sleep(min(1.8, max(0.35, len(cleaned) * 0.04)))
