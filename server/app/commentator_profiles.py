...
def settings_for_commentator_profile(
    ...
    global_tts = settings.tts_provider.lower().replace("-", "_")
    if global_tts in {"mimo", "openai"}:
        return settings.model_copy(update={"tts_provider": global_tts})

    voice_id = os.getenv(profile.voice_env, "") if profile.voice_env else ""
    profile_tts = profile.tts_provider.lower().replace("-", "_")
    if not voice_id and profile_tts == "fish_audio":
        voice_id = settings.fish_audio_voice_id or ""
    if not voice_id and profile_tts == "elevenlabs":
        voice_id = settings.elevenlabs_voice_id or ""
    if not voice_id:
        fallback = global_tts if global_tts else "openai"
        return settings.model_copy(update={"tts_provider": fallback})

    update: dict[str, object] = {"tts_provider": profile_tts}
    if profile_tts == "fish_audio":
        update["fish_audio_voice_id"] = voice_id
    elif profile_tts == "elevenlabs":
