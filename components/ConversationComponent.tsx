...
import { NetdiskVideoPicker } from './NetdiskVideoPicker';
import type { Mp4FeedPublisher } from '@/lib/agora/publish-mp4-feed';
...
export default function ConversationComponent({
  ...
  const feedPublisherRef = useRef<Mp4FeedPublisher | null>(null);

  useEffect(() => {
    return () => {
      void feedPublisherRef.current?.stop();
      feedPublisherRef.current = null;
    };
  }, []);
    ...
    (async () => {
      try {
        const ai = await AgoraVoiceAI.init({
          ...
          renderMode: TranscriptHelperMode.CHUNK,
          enableLog: true,
        });
  ...
  const liveSubtitleZh = useMemo(() => {
    const streaming = currentInProgressMessage?.text?.trim();
    if (streaming) return streaming;
    const agentMessages = messageList.filter(
      (message) => String(message.uid) === agentUID,
    );
    return agentMessages.at(-1)?.text?.trim() ?? '';
  }, [agentUID, currentInProgressMessage, messageList]);
  ...
  const handleStartAi = useCallback(async () => {
    ...
    if (!isMediaFeedConnected) {
      toast.warning('尚无比赛画面', {
        description: '请先从网盘选择录像并完成推流，再开启 AI 解说。',
      });
      return;
    }
          ...
          {mediaUser ? (
                ...
                <div className="flex w-full max-w-2xl flex-col items-center gap-4 px-6 text-center">
                  ...
                  <p className="text-xs leading-5 text-white/55">
                    频道 {agoraData.channel} · 推流 UID {mediaUID}
                  </p>
                  <NetdiskVideoPicker
                    channel={agoraData.channel}
                    disabled={isAiStarting}
                    publisherRef={feedPublisherRef}
                    onPublished={() => {
                      toast.success('录像推流成功', {
                        description: '画面就绪后可点击 Start AI。',
                      });
                    }}
              ...
              {liveSubtitleZh ? (
                <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/55 to-transparent px-4 pb-4 pt-10">
                  <p className="text-center text-base font-medium leading-relaxed tracking-wide text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.9)]">
                    {liveSubtitleZh}
                  </p>
                </div>
              ) : null}
